...
save_dir = f'./v3/{DATA_TYPE}/{args.model_type}_{args.backbone}_{args.secs}_{args.t1}_{args.t2}{args.save_prefix}'
...
def preprocess_df(df):
    ...
    for name, group in grouped:
        ...
        if len(s) != 1:
            cnt += 1
            min_votes = min(int(0.3 * max_votes), 5)
    ...
    print('selected len by total_votes: ', len(df))
    ###################

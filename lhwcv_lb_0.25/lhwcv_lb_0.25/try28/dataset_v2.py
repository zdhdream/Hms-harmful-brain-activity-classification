...
class DataGenerator(Dataset):
    def __init__(self, data, specs=None, eeg_specs=None, raw_eegs=None, augment=False, mode='train',
        ...
        self.pairs = Pairs().pairs_map[ptype]
    ...
    def generate_raw(self, index):
        X = np.zeros((10000, len(self.pairs)), dtype='float32')
        y = np.zeros((6,), dtype='float32')
        ...
        if self.mode == 'train':
            if random.random() < 0.3:
                ...
                item = self.aux_infos_by_eeg_id[row.eeg_id]
                if random.random() < 0.3 and item['tag'] != 'case4':
                    ...
                    group = self.aux_infos_by_group_id[row.group_id]
                    item2 = random.sample(group, 1)[0]
                    eeg = self.raw_eegs[item2['eeg_id']]
                    offset = int(item2['eeg_label_offset_seconds']) * 200
        ...
        if self.mode == 'train' and random.random() < 0.5:
            cutoff_freq = random.randint(18, 22)

        X = self.butter_lowpass_filter(X, cutoff_freq=cutoff_freq, order=4)

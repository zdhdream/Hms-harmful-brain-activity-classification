...
from raw_model3 import ECAPA_TDNN,ECAPA_TDNNV2, HybridModel, HybridModelV2
...
parser.add_argument('--model_type', type=str, default='raw_spec2')
...
save_dir = f'./v28_0401/{DATA_TYPE}/{args.model_type}_{args.backbone}_{args.secs}_{args.t1}_{args.t2}_ptype{args.ptype}{args.save_prefix}'
...
def preprocess_df(df):
    if os.path.exists('./cache/'):
        print('use cache..')
        all_max_votes_rows_df = pd.read_csv('./cache/all_max_votes_rows_df.csv')
        aux_infos_by_group_id = pickle.load(open('./cache/aux_infos_by_group_id.pkl','rb'))
        aux_infos_by_eeg_id = pickle.load(open('./cache/aux_infos_by_eeg_id.pkl', 'rb'))
    ...
    print('selected len by missing ratio: ', len(df2))

    # aux = pd.read_csv('/home/hw/m2_disk/kaggle/data/hms-harmful-brain-activity-classification/train_tag_with_loss.csv')
    # df2['loss'] = aux['loss'].values
    #
    # df2 = df2[df2['loss'] < 0.3]
    df = df2.copy().reset_index(drop=True)

    #print('selected len by loss: ', len(df))
    ...
    os.mkdir('./cache/')
    print('save cache..')
    all_max_votes_rows_df.to_csv('./cache/all_max_votes_rows_df.csv')
    pickle.dump(aux_infos_by_group_id, open('./cache/aux_infos_by_group_id.pkl','wb'))
    pickle.dump(aux_infos_by_eeg_id, open('./cache/aux_infos_by_eeg_id.pkl', 'wb'))
...
def build_model(backbone,
                ...
                num_mels=128,
                width=300):
    if MODEL_TYPE == 'hybrid2':
        return HybridModelV2(backbone,
    ...
    if MODEL_TYPE == 'raw_spec':
        return ECAPA_TDNN(backbone,
                          n_fft, win_length, num_mels, width).cuda()
    if MODEL_TYPE == 'raw_spec2':
        return ECAPA_TDNNV2(backbone,
                          n_fft, win_length, num_mels, width).cuda()
...
if not submission:
    ...
    for i in range(5):
        ...
        all_true_stage1.append(val[TARGETS].values)
    ...
    logger.write(f'scores stage1 mean: {np.mean(scores_stage1)}')
    logger.write(f'scores stage2 mean: {np.mean(scores_stage2)}')

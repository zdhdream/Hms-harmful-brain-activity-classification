...
def get_path_label(val_fold, train_all: pd.DataFrame):
    ...
    for label_id in train_all["label_id"].values:
        img_path = f"{TRAIN_SPEC_SPLIT}/{label_id}.npy"
...
def get_transforms(CFG):
    train_transform = A.Compose([
        # A.ElasticTransform(alpha=1.04, sigma=31.54, alpha_affine=30.87, interpolation=1, border_mode=4,
...
def train_one_fold(CFG, val_fold, train_all, output_path):
    ...
    MODEL_PATH = f"resnet34d/resnet34d_ra2-f8dcfcaf.pth"
    model = HMSHBACSpecModel(
        model_name=CFG.model_name, pretrained=True, num_classes=6, in_channels=1, path=None)
    model.to(device)
...
def main():
    ...
    for spec_id, df in tqdm(train.groupby("spectrogram_id")):
        spec = pd.read_parquet(f"{TRAIN_SPEC}/{spec_id}.parquet")

        spec_arr = spec.fillna(0).values[:, 1:].T.astype("float32")  # (Hz, Time) = (400, 320)
        # 获取频谱图的偏移量
        for spec_offset, label_id in df[['spectrogram_label_offset_seconds', "label_id"]].astype(int).values:
            spec_offset = spec_offset // 2
            split_spec_arr = spec_arr[:, spec_offset: spec_offset + 300]  # (400, 300)   600s -> 10min 挑选10min频谱图
            np.save(f"{TRAIN_SPEC_SPLIT}/{label_id}.npy", split_spec_arr)

...
def train_one_fold(CFG, fold, train_index, val_index, train_index2, val_index2, train_data, train_data2, spectrograms,
    ...
    train_data = train_data.copy()  # 高质量数据 kl<5.5
    train_data_2 = train_data2.copy()  # 低质量数据 kl>=5.5
    print('#' * 25)
    ...
    train_dataset = HMSHBASpecDataset(train_data.iloc[train_index],
                                      specs=spectrograms,
                                      eeg_specs=all_eegs,
                                      transform=train_transform, phase='train')

    val_dataset = HMSHBASpecDataset(train_data.iloc[val_index],
                                    specs=spectrograms,
                                    eeg_specs=all_eegs,
                                    transform=val_transform, phase='val')
    ...
    # 利用模型1为低质量数据打上伪标签(标签修正)
    test_dataset1 = HMSHBASpecDataset(train_data_2.iloc[train_index2],
                                      specs=spectrograms,
                                      eeg_specs=all_eegs,
    ...
    train_data_2.loc[train_index2, CLASSES] /= 2
    train_data_2.loc[train_index2, CLASSES] += pseudo_labels_2 / 2

    # pseudo labels for high quality data (50% of data) 对 50% 的带有高质量标签的数据
    train_data_3 = train_data
    test_dataset2 = HMSHBASpecDataset(train_data_3.iloc[train_index],
                                      specs=spectrograms,
                                      eeg_specs=all_eegs,
                                      transform=train_transform, phase="test")
    ...
    train_data_3.loc[train_index, CLASSES] /= 2
    train_data_3.loc[train_index, CLASSES] += pseudo_labels_3 / 2
    ...
    # 同时包含高质量数据于低质量数据的标签修正
    train_dataset2 = HMSHBASpecDataset(
        pd.concat([train_data_3.iloc[train_index_60], train_data_2.iloc[train_index2]]),
        specs=spectrograms,
        eeg_specs=all_eegs,
        transform=train_transform,
        phase="train")
    val_dataset2 = HMSHBASpecDataset(
        pd.concat([train_data_3.iloc[valid_index_60], train_data_2.iloc[val_index2]]),
        specs=spectrograms,
        eeg_specs=all_eegs,
...
def main():
    ...
    high_quality_df = train[(train['kl'] <= 5.5) & train['kl'] > 0].reset_index(drop=True)
    low_quality_df = train[train['kl'] > 5.5].reset_index(drop=True)

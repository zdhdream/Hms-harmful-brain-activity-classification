...
class HMSHBACSpecModel(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(3 * in_features, in_features)
        ...
        self.wave_block1 = Wave_Block(in_features*48, 160, 4, 3)
        self.wave_block2 = Wave_Block(160, 320, 3, 3)
        self.wave_block3 = Wave_Block(320, 640, 2, 3)
        self.wave_block4 = Wave_Block(640, 1280, 1, 3)
        self.LSTM = nn.LSTM(input_size=1280, hidden_size=640, num_layers=3, batch_first=True, bidirectional=True)
...
def train_one_fold(CFG, fold, train_index, val_index, train_all, spectrograms, all_eegs, output_path):
    ...
    warmup_steps = CFG.epochs/10 * len(train_loader) // 1
    num_total_steps = CFG.epochs * len(train_loader) // 1
    num_cycles = 0.49
    ...
    for epoch in range(1, CFG.epochs + 1):
        ...
        with tqdm(train_loader, leave=True) as pbar:
            for idx, batch in enumerate(pbar):
                ...
                optimizer.zero_grad()
                with amp.autocast(use_amp, dtype=torch.half):
                    y = model(x1, x2, x3)
                    loss = loss_func(y, t)
        ...
        for batch in tqdm(val_loader):
            ...
            with torch.no_grad(), amp.autocast(use_amp, dtype=torch.half):
                y = model(x, x, x)

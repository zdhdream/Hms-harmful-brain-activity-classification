...
class CFG:
    ...
    model_name = "tf_efficientnet_b3"  # tf_efficientnet_b5
    in_channels = 3
    head_dropout = 0.2
...
class HMSHBASpecDataset(torch.utils.data.Dataset):
    ...
    def make_Xy(self, new_ind):
        ...
        img = self.specs[row.spec_id][:, :].T  # (256, 400 or ???)  (400, 2458)
    ...
    def spec_mask(self, img, max_it=4):
        ...
        while count < max_it and torch.rand(1) > self.aug_prob:
            s = torch.randint(0, CFG.IMG_SIZE[0] - CFG.IMG_SIZE[0] // 16, (1,))[0]  # 选择mask的起始位置
            h = torch.randint(CFG.IMG_SIZE[0] // 32, CFG.IMG_SIZE[0] // 16, (1,))[0]  # 选择mask的高度
            new[s:s + h] *= 0
            count += 1
...
class HMSHBACSpecModel(nn.Module):
    ...
    def __init__(
        ...
        pretrained_cfg['file'] = r"/root/.cache/torch/hub/checkpoints/tf_efficientnet_b3_ns-9d44bf68.pth"
    ...
    def operate(self, x):
        ...
        x1 = torch.concatenate(x1, dim=2)  # (bs, 1, 1280, 512)
        # eeg spectrograms
...
def train_one_fold(CFG, fold, train_index, val_index, train_all, spectrograms, all_eegs, output_path):
    ...
    for epoch in range(1, CFG.epochs + 1):
        ...
        for idx, batch in enumerate(tqdm(train_loader)):
            ...
            optimizer.zero_grad()
            with amp.autocast(use_amp, dtype=torch.float16):  # torch.bfloat16
                y = model(x1, x2, x3)
                loss = loss_func(y, t)
